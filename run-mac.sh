...
# Check if the conda environment 'aimengpt' exists
if conda env list | grep -q 'aimengpt'; then
    echo "Conda environment 'aimengpt' already exists."
else
    echo "Creating a conda environment called 'aimengpt'..."
    conda create -n aimengpt python=$(python3 --version | cut -d ' ' -f 2)
fi

# Check if the conda environment 'aimengpt' is active
if [[ "$(conda info --envs | grep '*' | awk '{print $1}')" != "aimengpt" ]]; then
    echo "Activating the conda environment 'aimengpt'..."
    conda activate aimengpt
else
    echo "Conda environment 'aimengpt' is already active."
fi
